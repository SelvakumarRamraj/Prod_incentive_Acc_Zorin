...
Public Class Frmbarcoderemote1
    ...
    Private Sub Frmbarcoderemote1_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
    ...
    Private Sub Frmbarcoderemote1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub loadsetcont()
        ...
        msql = "declare @sqlstr varchar(max)" & vbCrLf _
             ...
             & " when charindex('SUIT',t1.U_ItemGrp)>0 then 'SUITING'" & vbCrLf _
             & " when charindex('TOWEL',t1.U_ItemGrp)>0 then 'TOWEL' end as ugrp " & vbCrLf _
             & "from ITT1 as t0 with (nolock) " & vbCrLf _
             & "left join [@INS_OPLM] as t1 with (nolock) on t1.U_ItemCode=t0.code " & vbCrLf _
             & "left join [@INCM_SZE1] as tz with (nolock) on tz.U_Name=t1.U_Size" & vbCrLf _
             & "group by case when charindex('DHOTI',t1.U_ItemGrp)>0 then 'DHOTI' " & vbCrLf _
             & " when charindex('SHIRT',t1.U_ItemGrp)>0 then 'SHIRTING' " & vbCrLf _
             & " when charindex('SUIT',t1.U_ItemGrp)>0 then 'SUITING' " & vbCrLf _
             & " when charindex('TOWEL',t1.U_ItemGrp)>0 then 'TOWEL' end ) as k order by k.UGrp" & vbCrLf
        ...
        msql = msql & "set @sqlstr='insert into contentbom(code,dhoti,shirting,suiting,towel) select father,'+ @subjtot+ '  from ( " & vbCrLf _
                 ...
                 & " from ITT1 as t0 with (nolock) " & vbCrLf _
                 & "left join [@INS_OPLM] as t1 with (nolock) on t1.U_ItemCode=t0.code " & vbCrLf _
                 & "left join [@INCM_SZE1] as tz with (nolock) on tz.U_Name=t1.U_Size ) p " & vbCrLf _
                 & "pivot(max(mksize) for ugrp IN('+@subjtot+')) AS p1'  " & vbCrLf _
                 & "execute(@sqlstr)"
    ...
    Private Sub loaddatadet()
        ...
        '     & "from " & Trim(mfil) & " as t1 with (nolock) " & vbCrLf _
        '     & "left join " & Trim(mfildet) & " as t0 with (nolock) on t0.docentry=t1.DocEntry " & vbCrLf _
        '     & "left join (select b.CardCode, CASE when len(rtrim(ltrim(u_salpricecode)))>0 and u_salpricecode IS not null then b.u_salpricecode else c.state end as state from OCRD b with (nolock) " & vbCrLf _
        '     & "     left join CRD1 c with (nolock) on c.CardCode=b.cardcode and (c.Address='Office' or c.Address='Ship') and c.AdresType='B') cr on cr.CardCode=t0.CardCode " & vbCrLf _
        '     & "left join barcodebom as bb with (nolock) on rtrim(BB.code)=rtrim(t1.itemcode) " & vbCrLf _
        '     & "Left Join " & vbCrLf _
        ...
        '     & "left join [@INS_OPLM] as t0 with (nolock) on t0.DocEntry=t1.DocEntry " & vbCrLf _
        '     & "left join [@INS_PLM2] as t2 with (nolock) on t2.DocEntry=t1.docentry and t2.U_RowID=t1.lineid" & vbCrLf _
        '     & "left join [@INCM_SZE1] as tz with (nolock) on tz.U_Name=t0.u_size" & vbCrLf _
        '     & "left join OITM as it with (nolock) on it.ItemCode=t0.U_ItemCode where t0.U_ItemCode is not null " & vbCrLf _
    ...
    Private Sub crtmptabst()
        ...
        msql4 = "Exec insertrbartemp " & Val(txtbno.Text) & ",'" & Trim(cmbprnon.Text) & "','" & Trim(brlinkserver) & "','" & Trim(brdbnam) & "'"
        Dim rCMD As New OleDb.OleDbCommand(msql4, con)
        ...
        'msql4 = "insert into bartemp select * from " & Trim(linkserver) & "." & Trim(dbnam) & ".dbo.bartemp with (nolock) where docnum= " & Val(txtbno.Text)
        ...
        'Dim riCMD As New OleDb.OleDbCommand(msql4, con3)
    ...
    Private Sub crtmptab()
        ...
        msql = " insert into bartemp (docentry,docnum,linenum,docdate,cardcode,cardname,itemcode,quality,u_size,quantity,boxqty,state,u_catalgcode,u_itemgrp,uom,box,u_length,u_width,u_style,rate,boxmrp,mrp,U_Remarks,u_brand,invntitem,u_prntname,msize,msize2,mfd,barcode,barcode1,barcode2,barcode3,drbarcode,dhoti,shirting,suiting,towel,bompcs,u_subgrp6,color,mtrstot,u_catalogname,autocode,contdhoti,contshirting,contsuiting,conttowel,contperfume,contbelt,contrate,contpcs) " & vbCrLf _
            ...
            & "left join (select b.CardCode, CASE when len(rtrim(ltrim(u_salpricecode)))>0 and u_salpricecode IS not null then b.u_salpricecode else c.state end as state from OCRD b with (nolock) " & vbCrLf _
            & "            left join CRD1 c with (nolock) on c.CardCode=b.cardcode and (c.Address='Office' or c.Address='Ship') and c.AdresType='B') cr on cr.CardCode=t0.CardCode" & vbCrLf _
            & "left join barcodebom as bb with (nolock) on BB.code=t1.itemcode " & vbCrLf _
            & "left join contentbom as cc with (nolock) on cc.code=t1.itemcode " & vbCrLf _
            & "Inner Join " & vbCrLf _
            ...
            & "Inner join [@INS_OPLM] as t0 with (nolock) on t0.DocEntry=t1.DocEntry " & vbCrLf _
            & "Inner join [@INS_PLM2] as t2 with (nolock) on t2.DocEntry=t1.docentry and t2.U_RowID=t1.lineid" & vbCrLf _
            & "Inner join [@INCM_SZE1] as tz with (nolock) on tz.U_Name=t0.u_size" & vbCrLf _
            & "Inner join OITM as it with (nolock) on it.ItemCode=t0.U_ItemCode where t0.U_ItemCode is not null " & vbCrLf _
            & " group by it.invntitem, t0.u_itemcode,t0.u_itemname,t1.u_catalgcode,t0.u_itemgrp,it.salunitmsr,it.salpackun,t0.u_size,tz.u_length,tz.u_width,case when t0.u_style IS null then ISNULL(t0.u_style,'') else  t0.u_style end ,case when t2.u_state='Other' then 'OS' else t2.U_State end ,t2.U_SelPrice,t2.u_mrp,convert(nvarchar(max),t1.U_Remarks),t1.u_brand,it.u_subgrp5,it.u_subgrp6,t1.u_prntname " & vbCrLf _
    ...
    Private Sub txtbno_Move(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtbno.Move
    ...
    Private Sub barprnsap1()
        ...
        For i = 1 To flx.Rows - 1
            ...
            If Len(Trim(flx.get_TextMatrix(i, 0))) > 0 Then
                ...
                'Header
                Dim CMD As New OleDb.OleDbCommand(msql, con3)
                If con3.State = ConnectionState.Closed Then
                    con3.Open()
                End If
                ...
                'Detail
                Dim CMD1 As New OleDb.OleDbCommand(msql2, con3)
                If con3.State = ConnectionState.Closed Then
                    con3.Open()
                End If
                    ...
                    If DR1.HasRows = True Then
                        While DR1.Read
                            ...
                            If Len(Trim(DR1.Item("ldata"))) > 0 Then
                                Dim cmd4 As New OleDb.OleDbCommand(msql, con3)
                                If con3.State = ConnectionState.Closed Then
                                    con3.Open()
                                End If
                ...
                'footer
                Dim CMD2 As New OleDb.OleDbCommand(msql2, con3)
                If con3.State = ConnectionState.Closed Then
                    con3.Open()
                End If
                    ...
                    If DR2.HasRows = True Then
                        While DR2.Read
                            ...
                            If Len(Trim(DR2.Item("ldata"))) > 0 Then
                                ...
                                Dim cmd5 As New OleDb.OleDbCommand(msql, con3)
                                If con3.State = ConnectionState.Closed Then
                                    con3.Open()
                                End If
    ...
    Private Sub barprnsapmulti2()
            ...
            If Len(Trim(flx.get_TextMatrix(i, 0))) > 0 Then
                ...
                For k = 1 To (Val(flx.get_TextMatrix(i, 4)))
                    ...
                    If mksno = Val(txtstickcol.Text) Then
                        ...
                        msql = "SELECT  [docentry],[date],[linenum],[headtype],[firstdet],[lrow],[lcol],[fontdet],[ldata],[secdet] FROM bardet where docentry=" & Val(txtdocno.Text) & " and headtype='H' order by docentry,linenum"
                        'Header
                        Dim CMDh As New OleDb.OleDbCommand(msql, con3)
                        If con3.State = ConnectionState.Closed Then
                            con3.Open()
                        End If
        ...
        If mksno2 = 1 Then
            msql2 = "SELECT  [docentry],[date],[linenum],[headtype],[firstdet],[lrow],[lcol],[fontdet],[ldata],[secdet] FROM bardet where docentry=" & Val(txtdocno.Text) & " and headtype='F' order by docentry,linenum"
            Dim CMD6 As New OleDb.OleDbCommand(msql2, con3)
            If con3.State = ConnectionState.Closed Then
                con3.Open()
            End If
        ...
        If chkprndir.Checked = False Then
            If MsgBox("Print", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                'Shell("print /d:LPT" & Trim(txtport.Text) & mdir, vbNormalFocus)
                'MsgBox("cmd.exe /c" & " type " & mdir & " > lpt" & Trim(txtport.Text))
                'Shell("cmd.exe")
                Shell("cmd.exe /c" & " type " & mdir & " > lpt" & Trim(txtport.Text))
        ...
        'p.Start("CMD.exe /C  type " & mdir & " > lpt" & Trim(txtport.Text))
    ...
    Private Sub cmbmont_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbmont.KeyPress

    End Sub

    Private Sub cmbmont_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbmont.SelectedIndexChanged
