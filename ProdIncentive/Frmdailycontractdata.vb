...
Public Class Frmdailycontractdata
    ...
    Private Sub delrec()
        If MsgBox("Delete anyway !", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            msql4 = "delete  from  prodcost.dbo.dailycontractdata where cdate='" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "'"
            Try
                executeQuery(msql4)
    ...
    Private Sub Frmdailycontractdata_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub attload()
        If msel > 1 Then
            ...
            If dt2.Rows.Count - 1 > 0 Then
                MSQL = "select nempno,nemp_id,vname,cdepartment,csno,isnull(rate,0) rate,isnull(qty,0) qty, case when isnull(amt,0)=0 then case when isnull(qty,0)>0 then isnull(qty,0)* isnull(rate,0) else 0 end else amt end amt from  prodcost.dbo.dailycontractdata where  cdate='" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "'  order by cdepartment,nempno "
        ...
        If MsgBox("Save ", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            Try
                ...
                For i = 0 To dg.Rows.Count - 1
                    msql4 = "select *  from  prodcost.dbo.dailycontractdata where cdate='" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "' and nempno=" & Val(dg.Rows(i).Cells(0).Value) & " and cdepartment='" & dg.Rows(i).Cells(3).Value & "'"
                    Dim dt4 As DataTable = getDataTable(msql4)
                    If dt4.Rows.Count > 0 Then
                        msql3 = "update  prodcost.dbo.dailycontractdata set cdepartment='" & dg.Rows(i).Cells(3).Value & "',csno=" & Val(dg.Rows(i).Cells(4).Value) & ",rate=" & Val(dg.Rows(i).Cells(5).Value) & " " & vbCrLf _
                               & ",qty=" & Val(dg.Rows(i).Cells(6).Value) & ",amt=" & Val(dg.Rows(i).Cells(7).Value) & "  where cdate='" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "' and nempno=" & Val(dg.Rows(i).Cells(0).Value)
                        'executeQuery(msql3)
                    Else
                        msql3 = "insert into  prodcost.dbo.dailycontractdata (nempno,nemp_id,cdate,vname,cdepartment,csno,rate,qty,amt) values (" & Val(dg.Rows(i).Cells(0).Value) & "," & Val(dg.Rows(i).Cells(1).Value) & ",'" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "'," & vbCrLf _
        ...
        If MsgBox("Save ", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            Try
                ...
                If dt3.Rows.Count - 1 > 0 Then
                    For k = 0 To dg.Rows.Count - 1
                        msql3 = "update  prodcost.dbo.dailycontractdata set cdepartment='" & dg.Rows(k).Cells(3).Value & "',csno=" & Val(dg.Rows(k).Cells(4).Value) & ",rate=" & Val(dg.Rows(k).Cells(5).Value) & " " & vbCrLf _
                                & ",qty=" & Val(dg.Rows(k).Cells(6).Value) & ",amt=" & Val(dg.Rows(k).Cells(7).Value) & "  where cdate='" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "' and nempno=" & Val(dg.Rows(k).Cells(0).Value) & " and cdepartment='" & dg.Rows(k).Cells(3).Value & "'"
                        executeQuery(msql3)
                    Next k
                    MsgBox("Updated Sucessfully!")
                    dg.Rows.Clear()
        ...
        If MsgBox("Save ", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ...
            Try
                For j = 0 To dg.Rows.Count - 1
                    msql2 = "insert into  prodcost.dbo.dailycontractdata (nempno,nemp_id,cdate,vname,cdepartment,csno,rate,qty,amt) values (" & Val(dg.Rows(j).Cells(0).Value) & "," & Val(dg.Rows(j).Cells(1).Value) & ",'" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "'," & vbCrLf _
                        & "'" & Trim(dg.Rows(j).Cells(2).Value) & "','" & Trim(dg.Rows(j).Cells(3).Value) & "'," & Val(dg.Rows(j).Cells(4).Value) & "" & vbCrLf _
                        & "," & Val(dg.Rows(j).Cells(5).Value) & "," & Val(dg.Rows(j).Cells(6).Value) & "," & (Val(dg.Rows(j).Cells(5).Value) * Val(dg.Rows(j).Cells(6).Value)) & ")"
                    executeQuery(msql2)
                    'MsgBox("Updated Sucessfully!")
                    'dg.Rows.Clear()
                Next j
    ...
    Private Function getdeptid(ByVal dept As String) As Integer
        Dim mpid As Integer
        MSQL = "select sno from rrcolor.dbo.departmentmast  where dept='" & dept & "'"
        Dim dt As DataTable = getDataTable(MSQL)
        If dt.Rows.Count > 0 Then
            'For Each row As DataRow In dt.Rows
    ...
    Private Function getdeptrate(ByVal mcsno As Integer) As Single
        Dim mrate As Single
        MSQL = "select rate from prodcost.dbo.processitemratemast  where csno=" & mcsno
        Dim dt As DataTable = getDataTable(MSQL)
        If dt.Rows.Count > 0 Then
            'For Each row As DataRow In dt.Rows
    ...
    Private Sub butdisp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butdisp.Click
        ...
        MSQL = "select b.nempno,b.nemp_id,b.vname,b.cdepartment,b.csno,c.rate from rrcolor.dbo.empDailysalary b " & vbCrLf _
              & " left join prodcost.dbo.processratemast c on c.nemp_id=b.nemp_id " & vbCrLf _
              & " where b.dot='" & Format(CDate(mskdate.Text), "yyyy-MM-dd") & "' and b.nemp_id not in (select nemp_id from prodcost.dbo.dailycontractdata)  and b.csno>=49 " & vbCrLf
